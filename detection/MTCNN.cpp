...
MTCNN::MTCNN(const std::vector<std::string> model_file, const std::vector<std::string> trained_file)
{
    ...
    for(int i = 0; i < model_file.size(); i++)
    {
        ...
        net.reset(new Net<float>(model_file[i], caffe::TEST));
...
void MTCNN::Predict(const cv::Mat& img, int i)
{
    ...
    boost::shared_ptr<caffe::Blob<float> > rect = net->blob_by_name(output_blob_names[0]);
    boost::shared_ptr<caffe::Blob<float> > confidence = net->blob_by_name(output_blob_names[1]);
...
void MTCNN::Predict(const std::vector<cv::Mat> imgs, int i)
{
    ...
    //confidence
    boost::shared_ptr<caffe::Blob<float> > confidence = net->blob_by_name(output_blob_names[i]);
    ...
    //landmarks
    if( i == 2){
        boost::shared_ptr<caffe::Blob<float> > points  = net->blob_by_name(output_blob_names[1]);
...
{
    boost::shared_ptr<caffe::Blob<float> > input_layer = nets_[i]->blob_by_name("data");
